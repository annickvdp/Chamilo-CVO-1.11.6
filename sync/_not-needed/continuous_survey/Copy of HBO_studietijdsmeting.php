...
<? 
//default data -----------------------------------------
$page_title = 'Studietijdmeting HBO' ;
...
#container{margin:0;width:100%;background:#FF3;} 
...
if ($_GET){
	...
	$selected_course_title = $_GET['course_title'];
	$selected_course_teacher = $_GET['course_teacher'];
...
while($row = mysql_fetch_array($result))
  ...
  if ($previous_category_code != $row['category_code']){  
  			...
  			$output_menu .= '<UL><LI>'.$row['category_code'].'</LI><UL class="studietijdmeting_submenu">'  ; 
  	}
...
inner join dokeos_main.course_rel_user on (course.code = course_rel_user.course_code) where course_rel_user.status <> 1 and code = '$selected_course_code' ; ";
 ...
 WHERE user.status <> 1 
   AND dokeos_course_$selected_course_code.survey_answer.answer_id IS NOT NULL
   AND dokeos_course_$selected_course_code.survey.code = '".$current_survey_Code."' 
...
WHERE
  user.status <> 1 AND survey_question.survey_question NOT LIKE 'Week Van' AND  dokeos_course_$selected_course_code.survey.code = '".$current_survey_Code."' 
GROUP BY
...
?></div><?
	...
	print '<br><strong><span class="estimated_hours">Gemiddlede uren geraamde: '.'80'.'</span></strong><br><br>' ;
...
so.addVariable("colours","#d01f3c,#356F33,#C79810,#3333CC");
