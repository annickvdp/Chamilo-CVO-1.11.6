...
*/
  ...
  function getFileList($dir, $recurse=false, $depth=false)
  {
    ...
    while(false !== ($entry = $d->read())) {
      ...
      if($entry[0] == ".") continue;
      if(is_dir("$dir$entry")) {
		  // four each found directory in /courses/....
		  //empty current course temp folder
	 
		  if( is_readable("$dir$entry"."/temp/")){ 
		  destroy("$dir$entry"."/temp/");
		  }
		  
		/*  // REMOVE OUTPUT TO SPEED UP SCRIPT
        $retval[] = array(
          "name" => "$dir$entry/",
  ...
  # single directory
   //$dirlist = getFileList("../courses/SHAUNTEST09/");
   $dirlist = getFileList("../courses");
    // $dirlist = getFileList("./temp/");
...
*/
 ...
 echo "all course temp folders eptied<br><br>";
 
 
  if( is_readable("../main/garbage/")){ 
		  destroy("../main/garbage/");
		  echo "garbage folder emptied<br><br>";
	}
