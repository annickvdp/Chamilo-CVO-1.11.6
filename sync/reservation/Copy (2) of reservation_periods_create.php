...
$schedule_profile = 'standard'; // standard(weekdays, sat morning), aula(evenings, sat morning)

//set default table values
$build_periods_for_item_or_category = 'item'; // 'item' or 'category'
$category_id=7; //Materiaal fotografie 
$id = NULL; //INT - autoincrement
$reservation_period_id = NULL; // ??? // MUST BE INCREMENTED
$item_id = NULL;  // unique item id - dokeos_reservation.item.id
...
if ($schedule_profile == 'standard'){ 
	...
	$periods_input[0]['start_td'] = $sunday_start_time = '';
	$periods_input[0]['end_td'] = $sunday_end_time = '';
...
$query = "SELECT item.id as item_id FROM dokeos_reservation.category inner join item on dokeos_reservation.category.id = dokeos_reservation.item.category_id
where category.id = 7";
