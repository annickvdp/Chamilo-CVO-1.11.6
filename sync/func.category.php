...
{global $dbs, $tbl_category;
 ...
 while($row = mysqli_fetch_array($rcs))
 {
  ...
  if(!addres){synclog_add('ERROR', 'CATEGORY ADD', 'Category with code "'.$cat_code.'" creation failed',$script);}
  else{synclog_add('INFO', 'CATEGORY ADD', 'Category with code "'.$cat_code.'" created',$script);}
 }
}
//--------------------------------------------------------------------------------------------------
//delete category
...
{global $dbs, $tbl_category;
 ...
 while($row = mysqli_fetch_array($rcs))
 {
  ...
  if(!$delres){synclog_add('ERROR', 'CATEGORY DELETE', 'Category with code "'.$cat_code.'" deletion failed', $script);}
  else{synclog_add('INFO', 'CATEGORY DELETE', 'Category with code "'.$cat_code.'" deleted', $script);}
 }
}
//--------------------------------------------------------------------------------------------------
